...
$("#add-harga-barang, #harga-barang-edit").mask("000.000.000", {
    reverse: true,
});
    ...
    tableBarang = $("#table-barang").DataTable({
        ...
        drawCallback: function () {
            ...
            $('input[type="search"]').addClass(
                "block w-full p-1 placeholder:text-sm placeholder:ps-1 placeholder:text-[#353935] border border-[#353935] rounded-md shadow-sm focus:outline-0",
            );
            // page entries
            $(".dt-length select").addClass(
                "flex p-1 border border-[#353935] rounded-md shadow-sm focus:border-[#353935] focus:outline-0",
            );
...
export function addBarang() {
    ...
    $("#add-barang").on("submit", function (event) {
        ...
        formData.append(
            "harga_barang",
            $("#add-harga-barang").val().replace(/\D/g, ""),
        );
        $.ajax({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
...
export function updateBarang() {
    $("#edit-barang").on("submit", function (event) {
        ...
        let text = button.find("span");
        button.attr("disabled", true);
        icon.removeClass("fa-regular fa-pen-to-square").addClass(
        ...
        formdata.append(
            "harga_barang",
            $("#harga-barang-edit").val().replace(/\D/g, ""),
        );
        $.ajax({
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            ...
            complete: function () {
                $("#edit_modal").removeClass("modal-open");
                button.removeAttr("disabled");
                icon.removeClass("fa-solid fa-spinner fa-spin").addClass(
...
export function chatRoom() {
    ...
    $("#button-chat-room").on("click", async function (event) {
        ...
        if (isOpen) {
            ...
            $("#button-chat-room")
                .find("i")
                .removeClass("fa-regular fa-comments")
            ...
            try {
                ...
                if (
                    ...
                    const userListHtml = response.data
                        .map(
                            (user) => `
               ...
               `,
                ...
                $("#all-user").html(
                    '<p class="text-center p-4">Error loading users</p>',
                );
            }
        } else {
            ...
            $("#button-chat-room")
                ...
                .addClass("fa-regular fa-comments");
        }
    });
}
